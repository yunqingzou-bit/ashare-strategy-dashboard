# -*- coding: utf-8 -*-
'''Table-only version (one row per picked stock) -> docs/tables.html'''
import json, os, sys
...
for p in picks:
    ...
    main_rows.append('<tr><td>' + p['date'][5:] + '</td><td class="stk">' + stock + '</td>' + cells
...
for p in sorted([x for x in picks if x['events']], key=lambda x: x['date']):
    ...
    ev_rows.append('<tr><td>' + p['date'][5:] + '</td><td class="stk"><b>' + p['code'] + '</b> ' + esc(p['name']) + '</td><td class="ev">'
...
@media print{body{background:#fff}.nav{display:none}.scroll{overflow:visible;box-shadow:none}th{position:static}h2{background:#eee;color:#000}table{font-size:9.5px}td.ev{min-width:0}}
'''
H = []
...
H.append('<div class="nav"><a href="index.html">← 返回看板</a><a href="#m1">主表 每只股票一行</a><a href="#m2">每日组合汇总</a><a href="#m3">事件专表</a><a href="#m4">统计评估</a></div>')
H.append('<h2 id="m1">主表 · 每只入选股票单独一行<span>' + str(len(main_rows)) + ' 行 · 胜率＝5个交易日中的上涨天数占比</span></h2>')
H.append('<div class="scroll"><table><thead><tr><th>日期</th><th>股票</th><th>D+1涨幅</th><th>D+2涨幅</th><th>D+3涨幅</th><th>D+4涨幅</th><th>D+5涨幅</th><th>5日累计</th><th>大事件</th><th>胜率</th></tr></thead><tbody>'
...
H.append('<div class="scroll"><table><thead><tr><th>日期</th><th>股票</th><th>大事件</th><th>D+1..D+5</th><th>5日累计</th></tr></thead><tbody>' + ''.join(ev_rows) + '</tbody></table></div>')
...
H.append('<footer>口径：①收益为“选股日收盘 → D+k收盘”的前复权持有收益，未扣手续费/印花税/滑点，未处理“次日一字板买不进”；②<b>胜率＝该股持仓5个交易日中的上涨天数占比</b>（例：4/5 = 80%），不足5日的最后几个交易日显示为“未满”；③板块分项用新浪行业成分股涨跌中位数替代同花顺行业+资金流，板质量分项用价格推导替代封单/首封时间；④全市场基准为同期全部个股（样本 ' + str(base['n5']) + '）等权统计；⑤历史统计，不构成投资建议。</footer>')
H.append('</div></body></html>')
os.makedirs('docs', exist_ok=True)
open('docs/tables.html', 'w', encoding='utf-8', newline=chr(10)).write(chr(10).join(H))
print('docs/tables.html', os.path.getsize('docs/tables.html'), 'bytes | 主表', len(main_rows), '| 每日', len(day_rows), '| 事件', len(ev_rows))
